...
interface Dish {
    id: string // Hyphen-separated human-readable identifier used in URLs
    name: string // Is not HTML-escaped
    description: string // Is not HTML-escaped
    tags: string[] // Is not HTML-escaped
    fields: string // Is not HTML-escaped
    imageURL: string // Ex. "/static/images/salmon.jpeg"
    reviews: Review[]
}
...
function loadContent() {
    ...
    if (window['reviews'] == 'SHOW') {
        ...
        history.replaceState(
            { command: 'rotateCarousel', index: carouselIndex * -1 },
            '',
            ''
        )
        ...
        history.replaceState(
            { command: 'rotateCarousel', index: carouselIndex },
            '',
            '/' + dishes[carouselIndex].id
        )
    }
...
function layoutCarousel() {
    ...
    var cellSize =
        document.querySelectorAll('.carousel-cell')[0].getBoundingClientRect()
...
function rotateCarousel(diff: number, pushState = true) {
    carouselIndex += diff

    var clampedCarouselIndex: number
    ...
    if (pushState) {
        history.pushState(
            { command: 'rotateCarousel', index: carouselIndex },
            '',
            '/' + dishes[clampedCarouselIndex].id
        )
    }
...
document
    ...
    .addEventListener('touchstart', (e: TouchEvent) => {
        ...
        if (
            imageScrollerClientRect.left < e.changedTouches[0].clientX &&
            ...
            document.querySelector('#image-scroller-swipe-hint')[
                'style'
            ].opacity = '0'
...
document.querySelector('#body').addEventListener('click', (e: MouseEvent) => {
    ...
    if (
        upButtonClientRect.left < e.clientX &&
    ...
    if (
        downButtonClientRect.left < e.clientX &&
...
document.querySelector('#button-desktop').addEventListener('click', () => {
    history.pushState(
        { command: 'reviews', subcommand: 'show' },
        '',
